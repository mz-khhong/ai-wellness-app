...
@AllArgsConstructor
@Schema(
    description = "매니저 도메인 모델 (테이블: managers)",
    example = "Manager(id=1, email=manager@example.com, name=매니저, status=ACTIVE)"
)
public class Manager {
    
    @Schema(
        description = "매니저 ID (Primary Key, 컬럼: id)",
        example = "1"
    )
    private Long id;
    ...
    @Schema(
        description = "매니저 이름 (컬럼: name, NOT NULL)",
        example = "매니저"
    )
    private String name;
    ...
    @Schema(
        description = "매니저 상태 (컬럼: status, Enum: ACTIVE/INACTIVE/SUSPENDED, 기본값: ACTIVE)",
        example = "ACTIVE",
