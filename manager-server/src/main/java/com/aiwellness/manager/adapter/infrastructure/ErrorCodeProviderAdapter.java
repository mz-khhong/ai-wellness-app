package com.aiwellness.manager.adapter.infrastructure;
...
/**
 * com.aiwellness.manager.adapter.infrastructure
 * <p>
 * ErrorCodeProviderAdapter
 * <p>
 * Manager 서버의 에러 코드를 제공하는 어댑터
 * <p>
...
public class ErrorCodeProviderAdapter implements ErrorCodeProviderPort {
    ...
    private static final String SERVER_NAME = "manager";
    private static final String DOMAIN_NAME = "manager"; // Manager 도메인
    ...
    public Map<String, List<ResponseCodeInfo>> getErrorCodesByDomain(Locale locale) {
        // Manager 서버의 에러 코드를 "manager" 비즈니스 영역으로 그룹화
        List<ResponseCodeInfo> managerCodes = Arrays.stream(ManagerErrorCode.values())
                .map(errorCode -> {
                    ...
                    log.debug("ManagerErrorCode: code={}, messageKey={}, message={}, locale={}", code, messageKey, message, locale);
                    return new ResponseCodeInfo(code, message, "ERROR");
                })
                .collect(Collectors.toList());
        
        Map<String, List<ResponseCodeInfo>> result = new HashMap<>();
        result.put(DOMAIN_NAME, managerCodes);
        return result;
    }
     ...
     */
    private String generateErrorCode(ManagerErrorCode errorCode) {
     ...
     */
    private int getOrderInSameStatus(ManagerErrorCode errorCode) {
        int httpStatus = errorCode.getStatus().value();
        long order = Arrays.stream(ManagerErrorCode.values())
    ...
    private String getTranslatedMessage(String messageKey, Locale locale) {
        try {
            ...
            if (message.equals(messageKey)) {
                log.warn("메시지 번역 실패: messageKey={}, locale={}, 번역된 메시지가 키와 동일함", messageKey, locale);
            }
            return message;
        } catch (Exception e) {
            log.error("메시지 번역 실패: messageKey={}, locale={}, error={}", messageKey, locale, e.getMessage(), e);
            return messageKey;
        }
    }
}
