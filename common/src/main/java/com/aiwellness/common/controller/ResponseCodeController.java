...
@Tag(name = "Response Codes", description = "응답 코드 정보 API (FE 연동용)")
public class ResponseCodeController {
    ...
    public ApiResponseWellness<ResponseCodeData> getResponseCodes() {
        // Locale 결정 (Accept-Language 헤더 기반)
        Locale locale = MessageUtil.getLocaleFromRequest();
        
        // Service를 통해 응답 코드 목록 조회 (구조화된 형태)
        ResponseCodeData data = responseCodeService.getAllResponseCodes(locale);
