...
@AllArgsConstructor
@Schema(
    description = "고객 도메인 모델 (테이블: customers)",
    example = "Customer(id=1, email=customer@example.com, name=고객, status=ACTIVE)"
)
public class Customer {
    
    @Schema(
        description = "고객 ID (Primary Key, 컬럼: id)",
        example = "1"
    )
    private Long id;
    ...
    @Schema(
        description = "고객 이름 (컬럼: name, NOT NULL)",
        example = "고객"
    )
    private String name;
    ...
    @Schema(
        description = "고객 상태 (컬럼: status, Enum: ACTIVE/INACTIVE/SUSPENDED, 기본값: ACTIVE)",
        example = "ACTIVE",
